...
def decide_on_datetime_format(*args, format_str='%Y-%m-%d %H:%M:%S'): #//
    if len(args)==2:
        ...
        CASE = "list"
        _list = args[0]
        datetime_list = [ts if isinstance(ts, pd.DatetimeIndex) else ts.to_pydatetime() if isinstance(ts, pd.Timestamp) else None for ts in _list]
        dt1 = next((item for item in datetime_list if item is not None), None)
        dt2 = next((item for item in reversed(datetime_list) if item is not None), None)
    elif len(args)==1 and isinstance(args[0], pd.DatetimeIndex):
        CASE=='DatetimeIndex'
        print('The hell do I do with a DatetimeIndex?')
    else:
        CASE=='Unknown'
        raise Exception('Can only process one list or two datetime-like values')
    ...
    if CASE=='strings':
        ...
        out = [dt.strftime(new_format_str) if dt is not None else None for dt in datetime_list]
    return out
    
    
def round_time_down(dt, round_to=300): #// move to be a property of .utils.DuckTypes class Timestamp
